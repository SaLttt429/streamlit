...
if mode == "🔋 放電模式 (使用電池)":
   ...
   # 舊的 st.info 刪除，改成以下兩行：
    st.warning(f"⚠️ 微積分工程解釋：在充電模式下，隨著亮度 $x$ 提高，導數 $\\frac{{dE}}{{dx}}$ 轉為負值且絕對值越來越大，這在物理上代表熱能累積造成的降速懲罰。圖表中的綠色最佳充電區間，正是透過求導尋求尋找出以下最佳條件的最高效亮度範圍：\n\n $$\n\\frac{{dE}}{{dx}} \\approx 0\n$$")
...
# ==============================================================================
   ...
   # 舊的 st.warning 刪除，改成以下兩行：
    st.warning(f"⚠️ 微積分工程解釋：在放電模式下...（原本的文字內容）...最高效亮度範圍：\n\n $$\n\\frac{{dP}}{{dx}} \\approx 0\n$$")
