...
if mode == "🔋 放電模式 (使用電池)":
    ...
    st.info("💡 **微積分工程解釋**：當你滑動亮度，紅色虛線的**斜率 (一階導數)**會即時變動。斜率越陡峭，代表高亮度下的邊際耗電代價越高。利用求導，系統能精準計算出續航力衰減率。")
...
# ==============================================================================
    ...
    st.warning("⚠️ **微積分工程解釋**：在充電模式下，隨著亮度 $x$ 提高，導數 $dE/dx$ 轉為**負值**且越來越大，這在物理上代表**熱能累積造成的降速懲罰**。綠色區間是透過求導找出 $\frac{dE}{dx} \approx 0$ 的最高效充電亮度範圍。")
